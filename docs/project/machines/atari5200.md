...
> routine puts its own color values back into GTIA every frame until the
...
| GTIA's CONSOL does not carry the computer's START/SELECT/OPTION and cassette behavior on the 5200 (`gtia.c` guards those on `machine_type != 5200`). | upstream `src/gtia.c` |
...
| The Altirra 5200 kernel that atari800 boots shows "Altirra 5200 ROM Kernel / Now playing:" and then the cartridge title from `$BFE8` before jumping to `$BFFE`; it leaves a display list up (the text stayed on screen while the probe recolored the playfield), so a program that never builds its own inherits the kernel's, exactly as a `.xex` inherits the OS's. | on screen |
| The kernel's vertical-blank NMI restores GTIA color registers every frame: the probe's COLBK write (made just after VCOUNT passed 64, ≈ scanline 128) colored only the lower half of the playfield and its single COLPF2 write never appeared; the same probe with `NMIEN ($D40E) = 0` before the loop colored the whole playfield and the text rows' background. So the 5200 has OS shadows too — at addresses not read here (*to verify*) — and clearing NMIEN is the verified way to take the registers over. | on screen, two builds of the probe |
...
center detent; POKEY reads them over one frame's scan (0–228) unless
...
- The BIOS has a VBI and color shadows of its own (verified above),
...
- Interrupts go through the BIOS's RAM vectors, whose addresses are *to
  ...
  BIOS's VBI stops rewriting the color registers (verified).

### Input is analogue and there is no keyboard

- `@8bitscript/input`'s "joystick" here is a pot pair with a dead zone
  around center and a POTGO restart every frame; the "buttons" are
