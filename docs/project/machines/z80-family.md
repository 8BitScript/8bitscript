...
> framebuffers with no sprites (Spectrum: 1 bpp + colour clash; CPC:
...
> (4 / 4 / 8), colour depth (1 / 1 / 4 bpp per sprite) and whether a
...
| **MSX**: CPU "Z80A or equivalent (clock 3.579545 MHz ±1 %)"; MSX1 RAM 8 K+, VRAM 16 K, TMS9918; MSX2 RAM 64 K+, VRAM 64 K or 128 K, V9938, RTC, 2 joysticks; screen modes Table 1.3 (TEXT1 40×24, TEXT2 80×24, MULTICOLOR 64×48, G1 32×24, G2 256×192, G3, G4 256×212, G5 512×212 4 col, G6 512×212, G7 256×212 256 col; sprite modes 1/2); "each 64K space is called a slot, which consists of four 16K areas called pages"; TMS9918: 16 colours max, 1 colour per sprite, no palette; V9938: 512-colour palette, 16 per sprite. | MSX2 Technical Handbook ch. 1 Tables 1.1–1.3, ch. 4 §1–3 |
| **V9938**: VRAM access via port 0 with auto-increment after a two-byte address on port 1 (bit 6 write flag) and R#14 for the 17-bit page; **sprite mode 1** (G1, G2, MC): 32 sprites, "up to 4 sprites … on a single horizontal line", collision S#0 bit 5, 5th-sprite flag bit 6 + number; **sprite mode 2** (G3–G7): 8 per line, colour per line, CC-bit OR mixing, 9th-sprite flag, collision coordinates S#3–S#5; sizes 8×8/16×16 (R#1 SI), MAG; R#23 vertical display offset; R#19 line interrupt (S#1 FH); R#9 NT = PAL 313 / NTSC 262 lines, IL interlace; S#0 F flag "when S#0 is read, this flag is reset"; per-mode characteristics blocks (TEXT1 6×8, 2 colours; G2 768 patterns 16 K; G4 32 K; G6/G7 need 128 K). | V9938 MSX-VIDEO Technical Data Book (map.grauw.nl) |
...
- **Amstrad CPC has no primary documentation in this note.** cpcwiki.eu
  ...
  per mode, the 27-colour palette values, the 64 µs/312-line frame, the
...
| Colour | 15 colours, 2 per cell | 15 fixed / 512-palette 16 | 64-palette 32 (2×16) / 4096 | 27-palette 16/4/2 per pixel | 15 fixed |
| Scroll | none | V9938 vertical only | X and Y, latched Y, lockable bars | CRTC start address (char steps) | none |
| Sprites | none | 32; **4/line** 1-colour (mode 1) — 8/line, per-line colour (V9938 mode 2) | 64; **8/line**, 15 colours, collision + overflow flags | none (Plus: 16 ASIC) | 32; **4/line** 1-colour |
...
- **Spectrum**: the only true framebuffer in the family, and a
  two-colours-per-cell one. A portable text/tile capability is a font
  blit plus an attribute write; a sprite capability is software with an
  attribute budget (a moving object recolours every cell it touches).
...
- **MSX1 / ColecoVision**: TMS9918 tables — the NES-shaped model
  (patterns, name table, sprite attributes) with 4 single-colour sprites
  ...
  SCREEN 2's "bitmap" is 768 patterns with 2 colours per 8×1 row — a
  tile mode wearing a bitmap coat.
- **MSX2 (V9938)**: the same tables plus real bitmap modes with a
  512-colour palette, vertical scroll, page flipping, block-copy commands
  and 8 multicoloured sprites per line. Treat MSX1 and MSX2 as two
...
- "32 sprites" (MSX, Coleco) and "64 sprites" (SMS) are attribute-table
  sizes; the constraints are 4 or 8 per scanline, 1 colour (TMS9918) or
