...
(`gb` and `gbc`), or the emulator and toolchain that
...
`gbc` is its own target, on the same SM83 backend as
`gb`. Double speed (`KEY1` and `stop`), the second VRAM bank,
banked WRAM, and the CGB palettes are facts of `gbc`. They are
not a hardware option of `gb`.

`gb` is the DMG, the MGB, and the SGB. `gbc` is a
...
Nothing. There is no `packages/gb`, no SM83 backend, no IR lowering
other than `packages/backend-6502` (which emits C and drives
`mos-*-clang`), and no `gb` profile, emulator flag or setup command.
...
| MAME machine names `gb`, `gbcolor`, `gbpocket`; `-str N` "will write a screenshot to the system's snapshot directory" at exit; `-video none`, `-sound none`, `-nothrottle`, `-cart`. | MAME `src/mame/mame.lst` (read raw), `commandline-all.html` (WebFetch) |
...
```
(nothing yet)
docs/roadmap.md                          Phase 8: gb and gbc, one SM83 backend, then z80
