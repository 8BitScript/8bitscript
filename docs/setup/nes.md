...
`8bs build --target nes` will produce an iNES ROM in the plainest NES cartridge
shape: 32K of program ROM, 8K of character ROM, no bank switching. Until
0.2.0 that command refuses.
...
comment on `FRAME_SYNC.nes` in `packages/compiler/src/mos`).
...
to edge — so a coloured frame is *drawn*: a ring
