...
| `@8bitscript/graphics` | Play a sprite compiled from a `.8bg` file: `graphics.place`, `graphics.update` once a frame. Hardware sprites on the C64, CHR/OAM on the NES, quadrant-blocks on the PET, a software glyph everywhere else. [docs/project/graphics.md](../project/graphics.md). |
| `@8bitscript/audio` | Play a sample or song compiled from a `.8ba` file: `audio.play`, `audio.music`, `audio.update` once a frame. SID, the NES APU, POKEY, or the PET VIA — or silence with a diagnostic on a machine that has no driver yet. [docs/project/audio.md](../project/audio.md). |
