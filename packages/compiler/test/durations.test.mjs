...
// (packages/compiler/src/fold), and the diagnostics it produces — including
// the `#` spelling's own.
...
test('#frames is its own token, so a program may declare its own frames', () => {
  clean('let frames: u8 = 60;');
  clean('function frames(n: uint): uint { return n; }');
  clean('import { frames } from "./x.8bs";');
  clean('function f(frames: u8): void { }');
  // Both in one program: the declared name and the compile-time function.
  clean('let frames: utinyint = 1;\nlet x: utinyint = #frames(0.5, seconds);');
});

test('a #name the compiler does not evaluate, or a #frames that is not called, is 8BS1030 at the spelling', () => {
  const src = 'let x: utinyint = #minutes(1);';
  const d = analyze(src, 't.8bs');
  assert.deepEqual(d.map((x) => x.code), ['8BS1030']);
  assert.equal(d[0].start, src.indexOf('#minutes'));
  assert.equal(d[0].length, '#minutes'.length);
  assert.match(d[0].message, /the compile-time functions are #frames\(\.\.\.\)/);

  const bare = analyze('let x: utinyint = #frames;', 't.8bs');
  assert.deepEqual(bare.map((x) => x.code), ['8BS1030']);
  assert.match(bare[0].message, /must be called: #frames\(1, seconds\)/);
});

test('a plain frames(...) is a runtime call like any other — the decimal literal is what gets flagged', () => {
  // No `#`, so this is an ordinary call to a function named `frames`, in a
  // place a call is legal. The only complaint is the decimal literal, which
  // has nowhere to live outside a compile-time duration.
  assert.deepEqual(
    codes('let x: utinyint = 0;\nexport function main(): void { x = frames(0.5, seconds); }'),
    ['8BS1009'],
  );
});
