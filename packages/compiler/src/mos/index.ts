...
export async function build(ir: IrProgram, options: BuildOptions): Promise<BuildResult> {
  ...
  // under x64sc: hello-world's 5_000_000-cycle capture was the boot screen).
  // Halting is three bytes and is the only way a program that draws once
  // and ends shows what it drew.
  const halt = image.entryIsVectored || image.endsByHalting;
  const endProgram: Directive[] = halt
    ? [
