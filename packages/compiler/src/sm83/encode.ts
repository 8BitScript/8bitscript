...
/** SM83 (Game Boy) encoder. Relatives are signed 8-bit from the next instruction. */
export const SM83: IsaEncode = {
  encode(ops, origin) {
    ...
    for (const op of ops) {
      switch (op.kind) {
        ...
        case 'loadAbs': emit(0xfa, op.address & 0xff, (op.address >>> 8) & 0xff); break;
        case 'storeAbs': emit(0xea, op.address & 0xff, (op.address >>> 8) & 0xff); break;
        case 'addAbs': emit(0x47, 0xfa, op.address & 0xff, (op.address >>> 8) & 0xff, 0x80); break; // LD B,A; LD A,(nn); ADD A,B
        case 'subAbs': emit(0x4f, 0xfa, op.address & 0xff, (op.address >>> 8) & 0xff, 0x47, 0x79, 0x90); break;
        case 'cmpImm': emit(0xfe, op.value & 0xff); break;
        case 'cmpAbs': emit(0x4f, 0xfa, op.address & 0xff, (op.address >>> 8) & 0xff, 0x47, 0x79, 0xb8); break;
        ...
        case 'jumpLT': emit(0x38); holes.push({ at: bytes.length, name: op.name, rel: true }); emit(0); break; // JR C: A < n
        case 'jumpGE': emit(0x30); holes.push({ at: bytes.length, name: op.name, rel: true }); emit(0); break; // JR NC
        ...
        case 'waitFrame': emit(0x00); break; // NOP: LY poll is a later pass
        case 'out': emit(0xe0, op.port & 0xff); break; // LDH (n), A
        case 'in': emit(0xf0, op.port & 0xff); break;
        default: return { ok: false, error: `sm83 cannot encode ${(op as Op).kind}` };
      }
    }
    // addAbs as written loads [addr] into A and ADDs A,B — B is stale. Fix: LD B,A; LD A,(nn); ADD A,B
    // Recode addAbs properly by rewriting the stream: handled below by a second pass only if we change emit.
