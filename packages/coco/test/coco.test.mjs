...
test('coco builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-coco-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'coco', entry], scratch);
...
test('coco screenshot skips when xroar is missing', async (t) => {
  if (!onPath('xroar')) {
    t.skip('xroar is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-coco-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'coco', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('xroar is present but could not capture');
      return;
    }
