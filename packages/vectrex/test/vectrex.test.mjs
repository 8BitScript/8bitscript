...
test('vectrex builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-vectrex-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'vectrex', entry], scratch);
...
test('vectrex screenshot skips when vecx is missing', async (t) => {
  if (!onPath('vecx')) {
    t.skip('vecx is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-vectrex-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'vectrex', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('vecx is present but could not capture');
      return;
    }
