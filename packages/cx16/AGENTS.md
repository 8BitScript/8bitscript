...
| **x16emu is not started captured.** The catalog's stock `run` flags carried `-capture` (mouse and keyboard confined to the window) from a side-bar refactor with no reason recorded; taken out 2026-09-21 so the pointer moves in and out of the emulator like any other window — Studio is a desk beside the editor, not a game — and ⇧⌘M (Ctrl+M on Linux/Windows) captures on request, x16emu's own default arrangement (its README, "Keyboard shortcuts"). A program that wants the mouse confined asks the person, not the launcher. | `package.json` (`run.x16emu`); `test/mouse.test.mjs`, `packages/cli/test/hardware.test.mjs` |
