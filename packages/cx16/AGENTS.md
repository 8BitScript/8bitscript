...
| **x16emu is started captured (`-capture`), on purpose.** Tried without on 2026-09-21 (#230) so the pointer could move in and out of the window like Studio's editor neighbour: an uncaptured x16emu feeds the KERNAL mouse *relative* motion, so the emulated pointer and the host pointer drift apart and tracking is unusable — reverted the same day. Captured, the two agree; ⇧⌘M (Ctrl+M on Linux/Windows) releases the mouse to leave the window and captures it again on the way back (x16emu README, "Keyboard shortcuts"). The flag had come in with a side-bar refactor with no reason recorded; this row is the reason. | `package.json` (`run.x16emu`); `test/mouse.test.mjs`, `packages/cli/test/hardware.test.mjs` |
| **`8bs run cx16 --web` is the same x16emu in a browser tab.** X16Community ships a WebAssembly build with each release (`x16emu_wasm-<tag>.zip`, `make wasm`, ROM embedded in `x16emu.data`); the CLI pins r49 by URL and SHA-256, unpacks it into `~/.cache/8bitscript/setup/x16emu-wasm/r49/` on first use (no Emscripten, no sudo), and serves it from loopback with the freshly built `.prg` and a page of its own. The page hands Emscripten the *native* argv (`-capture -ram 512 -prg program.prg -run` — the catalog's flags, the controller's, unchanged), not upstream's `x16emu.html`, whose loader only knows a handful of URL parameters. Confirmed 2026-09-21 under headless Chromium: Studio's `main-cx16.prg` boots (menubar, `INPUT KEYS MOUSE`, pointer); a click on the canvas takes the mouse through the Pointer Lock API and Esc gives it back — the browser's answer to the row above. The native build is r50 from source and the wasm is r49; nothing in `packages/cx16` has needed the difference yet — check again when bumping the pin. | `packages/cli/src/setup/cx16-web.mjs` (the pin), `packages/cli/src/web-emulator.mjs` (page, server), `packages/cli/test/{setup-cx16-web,web-emulator}.test.mjs` |
