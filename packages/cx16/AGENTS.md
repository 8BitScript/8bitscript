...
> a program while `cx16` is listed in `RELEASE_MACHINES`
...
| **x16emu starts with the mouse free (no `-capture`), on purpose — and it used to start captured, also on purpose.** Captured, the emulated pointer and the host pointer agree exactly; uncaptured, the KERNAL mouse gets relative motion and the two can drift. #230 removed the flag, #231 put it back for the drift, and on 2026-09-21 it came out again: in use, uncaptured tracking is decent enough for Studio, and the pointer being free to leave the window for the editor matters more. ⇧⌘M (Ctrl+M on Linux/Windows) captures when tracking has to be exact, and again releases; a click does *not* capture (src/video.c's SDL_MOUSEBUTTONDOWN only presses the emulated button; only `mousegrab_toggle()` grabs). `8bs run cx16` prints that shortcut once per launch. | `package.json` (`run.x16emu`); `test/mouse.test.mjs`, `packages/cli/test/hardware.test.mjs`; `packages/cli/src/run.mjs` (`CX16_MOUSE_NOTE`) |
| **`8bs run cx16 --web` is the same x16emu in a browser tab.** X16Community ships a WebAssembly build with each release (`x16emu_wasm-<tag>.zip`, `make wasm`, ROM embedded in `x16emu.data`); the CLI pins r49 by URL and SHA-256, unpacks it into `~/.cache/8bitscript/setup/x16emu-wasm/r49/` on first use (no Emscripten, no sudo), and serves it from loopback with the freshly built `.prg` and a page of its own. The page hands Emscripten the *native* argv (`-ram 512 -prg program.prg -run` — the catalog's flags, the controller's, unchanged), not upstream's `x16emu.html`, whose loader only knows a handful of URL parameters. Confirmed 2026-09-21 under headless Chromium: Studio's `main-cx16.prg` boots (menubar, `INPUT KEYS MOUSE`, pointer). The mouse follows the argv as in the row above: with `-capture`, a click on the canvas takes it through the Pointer Lock API and Esc gives it back (verified, also from a cross-origin frame); with the stock free launch the emulator's own grab toggle is a Pointer Lock request from a key press — **Ctrl+M, on a Mac too**: ⇧⌘M never reaches the emulator in the browser (verified). The native window keeps ⇧⌘M. The page says which mode it is in and posts it to a framing page. The native build is r50 from source and the wasm is r49; nothing in `packages/cx16` has needed the difference yet — check again when bumping the pin. | `packages/cli/src/setup/cx16-web.mjs` (the pin), `packages/cli/src/web-emulator.mjs` (page, server), `packages/cli/test/{setup-cx16-web,web-emulator}.test.mjs` |
...
packages/cli/src/web-emulator.mjs        8bs run cx16 --web: the WebAssembly x16emu, served on loopback with the .prg
packages/cli/src/setup/cx16-web.mjs      the pinned x16emu_wasm-r49.zip: tag, URL, SHA-256, the three files served
