...
test('gbc builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gbc-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'gbc', entry], scratch);
...
test('gbc screenshot skips when sameboy is missing', async (t) => {
  if (!onPath('sameboy')) {
    t.skip('sameboy is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gbc-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'gbc', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('sameboy is present but could not capture');
      return;
    }
