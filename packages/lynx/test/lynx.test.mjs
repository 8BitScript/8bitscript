...
test('lynx builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-lynx-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'lynx', entry], scratch);
...
test('lynx screenshot skips when mame is missing', async (t) => {
  if (!onPath('mame')) {
    t.skip('mame is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-lynx-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'lynx', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
