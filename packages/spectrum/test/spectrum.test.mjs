...
test('spectrum builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-spectrum-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'spectrum', entry], scratch);
...
test('spectrum screenshot skips when fuse is missing', async (t) => {
  if (!onPath('fuse')) {
    t.skip('fuse is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-spectrum-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'spectrum', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('fuse is present but could not capture');
      return;
    }
