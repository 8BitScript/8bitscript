...
//
// pet, c64, vic20, c128 and cx16 actually build in this release, alongside
// web (per RELEASE_MACHINES in build.mjs); the remaining three targets are
...
const PARKED_TARGETS = ['atari8', 'nes', 'mega65'];
...
for (const target of PARKED_TARGETS) {
  test(`${target}: --screenshot refuses to build — parked until a later release`, async () => {
    await withProbe(async (scratch, entry) => {
      ...
      assert.match(
        stderr,
        /is not a target in this release\. This release builds for pet, c64, vic20, c128, cx16 and web/,
