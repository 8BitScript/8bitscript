...
test('boot() refuses a parked target by name, the same as build()/run() do', async () => {
  ...
  const { result, stderr } = await capture(() => boot(['mega65']));
  assert.equal(result, 2);
  assert.match(stderr, /^8bs boot: 'mega65' is not a target in this release\. This release builds for pet, c64, vic20, c128, cx16 and web/);
});
