...
> 48×27 character grid (384×216 inner picture, exact 16:9) inside a
> 24-pixel border on a large mouse desktop, the C64's sixteen colors by
> default, one flat 64 KB page, and a logical frame clock at the
> project's `frameRate`. A `machine` hardware option rebuilds the same
> host as a 4:3 PET 2001, C64, or VIC-20 skin — each its own wasm, never
> one binary switching `#system()`. Treat it as a machine whose
...
- `packages/web/src/index.8bs` exports one namespace, `WebRegisters`: byte
  offsets into the program's wasm linear memory — `BORDER_OFFSET` 0,
  `BACKGROUND_OFFSET` 1, `CHAR_BASE` / `COLOR_BASE` / `INPUT_OFFSET` /
  `HOST_OFFSET` from `geometry.8bs` (1298 / 2594 / 2595 on the default
  48×27 host). That is the whole "hardware surface": an agreement between
  the package and the host about where in the program's own memory the
  picture, a one-byte input snapshot, and a host-status byte live. There
...
- `src/text.8bs` (behind `@8bitscript/text`) is a `Video.COLUMNS` /
  ...
  `text.putColor` writes `COLOR_BASE + cell`, `text.print`/`printNumber` write
...
- `8bs run web` (`packages/cli/src/web-runtime.mjs`) serves the `.wasm`, a
  page and a worker and opens the OS browser (`--no-open` prints the URL
  only). `--lan` is the default: HTTP on `0.0.0.0` (the same port as
  loopback, so a phone can actually open it) plus HTTPS on `0.0.0.0` so
  SharedArrayBuffer is legal; a phone HTTP request redirects to HTTPS.
  Safari will warn once. `--local` is loopback only. HTTP listens on port
  8008 by default (HTTPS on 8009); `--port n` picks another, `--port 0` an
  ephemeral port. The page is generated by this CLI in memory, not
  ...
  one exported function; the page paints the shared memory onto a 432×264
...
| Shared memory needs cross-origin isolation, so every response carries `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`. The gate is on *sharing* it, not on having it: without the headers, `new WebAssembly.Memory({shared: true})` still succeeds and a shared-memory module still instantiates, but `postMessage` of the `SharedArrayBuffer` — or of the `WebAssembly.Memory` — throws `DataCloneError: SharedArrayBuffer transfer requires self.crossOriginIsolated`. No worker arrangement dodges it: a worker inside a blocking `waitFrame()` never reaches its event loop, so it cannot be sent input by message either. | `web-runtime.mjs`, `ISOLATION_HEADERS` (read); the three `postMessage`/instantiate outcomes (ran, headless Chromium 153 on a page with no headers) |
| A host that cannot send those headers can load `coi.js`, which registers a service worker that adds them to its own origin's responses and reloads once. Verified against a static server sending none: `crossOriginIsolated` flips true and the program runs at `frameRate`. It is opt-in because `require-corp` then applies to the embedder's whole page. | `packages/cli/src/web-loader.mjs`, `renderCoiServiceWorker` (ran, headless Chromium 153) |
...
4. **Text grid** — 48×27 of 8×8 px (16:9) inside a 24 px border whose color is
...
3. **Native unit** — a virtual video chip made of what every 6502 target
   ...
   pixel-thinking.
4. **Text grid** — the default is 48×27 16:9; machine skins are geometry twins the way the PET does (`geometry.web.<tag>.8bs`). 40×25
...
13. **Profiles** — `hifi` (today's 48×27 16:9) plus `pet-2001` / `c64` /
    `vic20` 4:3 skins. Each is its own wasm. If Studio needs it,
...
```
packages/web/src/index.8bs               target package: WebRegisters — the offsets (0, 1, CHAR_BASE, COLOR_BASE, INPUT_OFFSET, HOST_OFFSET) the host and the input layer share
...
packages/cli/src/web-lan.mjs             `8bs run web --lan` (the default): LAN IPv4, port 8008, the self-signed cert, HTTPS on 8009
...
packages/cli/src/run.mjs                 8bs run web / --no-open / --lan (default) / --local → runInBrowser; --screenshot route
...
docs/web-embedding.md                    embedding: the bundle's files, the two headers per host, coi.js and what it costs, the border table, the mount()/<eightbit-screen> options
