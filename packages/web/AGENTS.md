...
- `8bs run web` (`packages/cli/src/web-runtime.mjs`) serves the `.wasm`, a
  ...
  Safari will warn once. `--local` is loopback only. HTTP binds an
  ephemeral port by default so two runs can coexist (`--port n` pins
  HTTP to n, HTTPS to n+1). The page is generated by this CLI in memory, not
