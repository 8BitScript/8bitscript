...
test('atari2600 builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-atari2600-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'atari2600', entry], scratch);
...
test('atari2600 screenshot skips when stella is missing', async (t) => {
  if (!onPath('stella')) {
    t.skip('stella is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-atari2600-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'atari2600', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('stella is present but could not capture');
      return;
    }
