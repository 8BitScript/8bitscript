...
test('gb builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gb-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'gb', entry], scratch);
...
test('gb screenshot skips when sameboy is missing', async (t) => {
  if (!onPath('sameboy')) {
    t.skip('sameboy is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gb-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'gb', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
