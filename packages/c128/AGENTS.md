...
- `src/input.8bs` and `src/pointer.8bs` are the C128 behind
...
- `FRAME_SYNC.c128` (`packages/compiler/src/mos`) is the C64's level driver
  ...
  **without the C64's `presync`**: a C128 program keeps the KERNAL's IRQ
  ...
  exists. `waitFrame()` itself still does not `sei` — input's `poll()`
  holds interrupts only across the CIA1 snapshot and the SID pots.
