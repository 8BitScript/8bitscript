...
test('pce builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-pce-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'pce', entry], scratch);
...
test('pce screenshot skips when mednafen is missing', async (t) => {
  if (!onPath('mednafen')) {
    t.skip('mednafen is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-pce-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'pce', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
