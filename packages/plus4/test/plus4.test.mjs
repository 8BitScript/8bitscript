...
test('plus4 builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-plus4-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'plus4', entry], scratch);
...
test('plus4 screenshot skips when xplus4 is missing', async (t) => {
  if (!onPath('xplus4')) {
    t.skip('xplus4 is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-plus4-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'plus4', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('xplus4 is present but could not capture');
      return;
    }
