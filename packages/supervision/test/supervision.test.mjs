...
test('supervision builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-supervision-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'supervision', entry], scratch);
...
test('supervision screenshot skips when mame is missing', async (t) => {
  if (!onPath('mame')) {
    t.skip('mame is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-supervision-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'supervision', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
