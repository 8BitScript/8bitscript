...
test('cpc builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-cpc-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'cpc', entry], scratch);
...
test('cpc screenshot skips when cap32 is missing', async (t) => {
  if (!onPath('cap32')) {
    t.skip('cap32 is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-cpc-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'cpc', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('cap32 is present but could not capture');
      return;
    }
