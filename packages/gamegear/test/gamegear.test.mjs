...
test('gamegear builds a probe program', async () => {
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gamegear-'));
  try {
    ...
    const { code, stderr } = await runCli(['build', '--target', 'gamegear', entry], scratch);
...
test('gamegear screenshot skips when mednafen is missing', async (t) => {
  if (!onPath('mednafen')) {
    t.skip('mednafen is not installed');
    return;
  }
  const scratch = await mkdtemp(join(tmpdir(), '8bs-gamegear-shot-'));
  try {
    const entry = join(scratch, 'main.8bs');
    await writeFile(entry, 'export function main(): void {}\n');
    const shot = join(scratch, 'out.png');
    const { code } = await runCli(['run', 'gamegear', entry, '--screenshot', shot, '--frames', '1'], scratch);
    if (code !== 0) {
      t.skip('mednafen is present but could not capture');
      return;
    }
